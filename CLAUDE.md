...
**Important**: The `make build` target has a duplicate build-frontend call (lines 34 and 21-24). Use `make build-backend` for production builds to avoid redundant frontend builds.
...
1. Frontend build generates files in `frontend/dist/` (via `npm run build`)
...
- Run via `npm run test:e2e`, `npm run test:e2e:ui`, or `npm run test:e2e:headed`
...
### Frontend
11. **Embedding Frontend**: Frontend changes require rebuilding the embedded Go files with `make build-frontend` or `npm run build` in the frontend directory.
...
17. **Makefile Target Issue**: The `build` target has a redundant `build-frontend` call. Use `build-backend` for clean production builds.
