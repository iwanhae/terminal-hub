...
**Linux/macOS:**
```bash
curl -sL "https://github.com/iwanhae/terminal-hub/releases/download/v1.0.1/terminal-hub_1.0.1_$(uname -s | tr '[:upper:]' '[:lower:]')_$(uname -m | sed 's/x86_64/amd64/' | sed 's/aarch64/arm64/').tar.gz" | tar xz && ./terminal-hub
```

**Windows (PowerShell):**
```powershell
Invoke-WebRequest -Uri "https://github.com/iwanhae/terminal-hub/releases/download/v1.0.1/terminal-hub_1.0.1_windows_amd64.zip" -OutFile "terminal-hub.zip"
